document.addEventListener('DOMContentLoaded', function () {
  ...
  const resetMilestoneButton = document.getElementById('resetMilestone');
  const versionValue = document.getElementById('versionValue');
  const MAX_INVITES_4HOURS = 1800;
  const MAX_INVITES_24HOURS = 7000;
  const MAX_INVITES_MILESTONE = 20000;
  const MAX_INVITES_REMAINING = 1800;
  const MAX_VISIBLE_INVITES = 750;
  const MAX_BAR_HEIGHT = 1680;  // Max value represented by px height for 1h bar
  const MAX_TEN_MINUTE_BARS = 24;
  const TEN_MINUTE_INTERVAL = 10 * 60 * 1000; // 10 minutes in milliseconds
  const TEN_MINUTE_BAR_MAX_VALUE = 300; // Max value represented by px height for 10m t-bar
  ...
  function translatePage() {
    ...
    document.getElementById('totalSinceMilestone').innerHTML = `${t.totalSinceMilestone}<span class="bold" id="totalSinceMilestoneValue">0</span><button id="resetMilestone" class="small-button">${t.reset}</button>`;
  ...
  loadLanguagePreference(); // Load the preferred language on startup
  ...
  resetMilestoneButton.addEventListener('click', () => {
  ...
  function updateVisibleInvites() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.scripting.executeScript({
        target: { tabId: tabs[0].id },
        func: () => {
          return document.querySelectorAll('div[aria-label="Pozvat"][role="button"]').length;
        }
      }, (results) => {
